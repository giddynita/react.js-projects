      ...
      <section className="my-12 width">
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-6">
          {about.map((info, index) => {
            const { icon, figure, name, link, page } = info
            return (
              <div
                key={index}
                className="flex flex-col gap-3.5 items-center pt-7"
              >
                <div className="text-3xl text-accent/80">{icon}</div>

                <div className="flex gap-0.5">
                  {Array.from(figure).map((digit, index) => {
                    return (
                      <p
                        key={index}
                        className="text-3xl text-center font-extrabold bg-accent/80 text-white/95 rounded-md w-8 py-0.5 figure"
                      >
                        {digit}
                      </p>
                    )
          ...
          return (
            ...
            >
              <img
                src={img}
                alt="about"
                className="w-full h-full transition-all duration-300 img"
              />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 border border-r-0 border-b-0">
          {partnerImgs.map((img, index) => {
            return (
              <figure
                key={index}
                className="border border-l-0 border-t-0 partner h-[80px] py-5 relative cursor-pointer"
              >
                <img
                  src={img}
                  alt="deCoda"
                  className="h-full object-contain mx-auto pImg opacity-100 visible transition-all duration-300 origin-center"
                />
                <figcaption
                  className="absolute top-0 w-full h-full flex flex-col items-center justify-center opacity-0 invisible transition-all duration-300  origin-center pCap"
                  onClick={comingSoon}
                >
